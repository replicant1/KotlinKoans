...
class EffectiveDate<R> : ReadWriteProperty<R, MyDate> {
    ...
    operator override fun getValue(thisRef: R, property: KProperty<*>): MyDate {
        println("Into getValue: thisRef=${thisRef}, property=${property.name}")
        val c = Calendar.getInstance()
        c.timeInMillis = timeInMillis ?: 0
        println("In getValue: c.timeInMillis=${c.timeInMillis}, calendar c=${c}")
    ...
    operator override fun setValue(thisRef: R, property: KProperty<*>, value: MyDate) {
        println("Into setValue: thisRef=${thisRef}, property=${property.name}, value=${value}")
        timeInMillis = value.toMillis()
        println("Store value: timeInMillis=${timeInMillis}")
    }
}
